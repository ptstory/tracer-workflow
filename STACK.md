...
- Skill invocation on ChatGPT is not verifiable by output shape. (session) On 2026-09-10 a request for last30days prompt evals produced skill-shaped output, then stated plainly that it had not invoked the installed last30days skill and had used direct web research instead. Scope: that one skill, that surface, that date. to-issues, triage, agent-brief, and review-gate on the same surface remain untested.
- Likely cause is surface, not plan tier. (reported) OpenAI's current documentation contradicts itself — the Skills article lists Business/Enterprise/Healthcare/Edu, a separate Plugins FAQ says Personal Skills in Work are available on all paid plans except Free and Go, and OpenAI's own guidance is to check Plugins → Skills in the surface actually being used. Skills are documented as living in Work and Codex rather than ordinary chat, and web and desktop do not sync skills.
- Workaround adopted: ChatGPT-side skills rebuilt as Custom GPTs. (session) Custom GPT instructions are always in context, unlike knowledge files which are retrieved, so rules pasted into Instructions are genuinely loaded. New risk: a GPT's copy of a SKILL.md goes stale silently when the on-disk version changes, with nothing surfacing the drift. Test for whether rules are loaded — ask the GPT to quote a distinctive constraint verbatim rather than paraphrase its purpose.
- Matt Planning GPT establishes an invocation contract. (disk) It resolves mattpocock/skills main, pins that exact commit for the whole invocation, fetches canonical SKILL.md at that SHA, and fails explicitly rather than reconstructing from memory. Verified end to end on 2026-09-10: mattpocock/skills@3cca18b, to-tickets/SKILL.md blob e868c831fcfb1e124e010bcdf84a429ec879160f, decoded frontmatter name: to-tickets, private-repo read against ptstory/tracer-workflow#125, private-repo write against ptstory/thread-atlas#136. Open gap: main is re-resolved each invocation, so two plans made days apart can use different upstream methodology with nothing flagging the difference; the durable provenance line in published tickets records it after the fact but only if compared.
- Repo already matched Matt's conventions before the alignment decision. (disk) A setup-matt-pocock-skills audit on 2026-09-10 found AGENTS.md, CONTEXT.md, and all three docs/agents/ files present and correct, the five canonical triage labels identity-mapped, and no monorepo signals. Concrete gaps: none. PR #129 commits those files; it is not a migration.
- Architecture skills declined with reasons. (disk) An audit of the source rather than the workflow docs found the dependency graph shallow — gate-packet → gate-state → lib/verdict, review-gate-poller → lib/verdict — across roughly 6k lines in one package with tests beside each tool. codebase-design, improve-codebase-architecture, and wayfinder were all assessed as unwarranted. Recorded trigger for revisiting: tooling/doctor/doctor.ts (892 lines) developing multiple change axes. PR #122's five review rounds all landed in that file, which is arguable evidence the trigger has been met.
- Correction: the per-directory ablation deltas measure advertising cost in the system prompt, not which roots are loadable. (session) A 2026-09-10 OpenCode session loaded skills from all three roots in one run — ask-matt from ~/.claude/skills, setup-matt-pocock-skills and domain-modeling from ~/.agents/skills, finishing-a-development-branch and verification-before-completion from ~/.config/opencode/skills/superpowers. Advertised-in-prompt and loadable-on-demand are separate properties.

## 2026-09-05

Findings:
- Correction to the 2026-08-30 entry: its raw `session.model` history omits an `openai/gpt-5.6-terra` orchestrator session on 2026-07-19 at `medium` variant (`ses_085d89cb7ffejozz97emdmYUuR`, directory `~/Code/vibecoding/whatimeant`). (disk) The query behind that line was evidently bounded to August and written up as full history. Terra had prior use on this seat, n=1, six weeks before the current window and on product code rather than tracer-workflow. Do not cite the 2026-08-30 model history as complete.
- Terra window session distribution: the switch took effect 2026-08-31 (5 sessions), not 2026-09-01 as the 2026-08-30 entry's plan implies. (disk) Parent orchestrator sessions by day: 08-31 five, 09-01 thirty-one, 09-02 four, 09-03 two, 09-04 three, 09-05 three. 2026-09-01 is the config-audit day and is 63 percent of the window's parent sessions; exclude it from any window average. Usable n after exclusion is approximately 12 parent sessions.
- Variant contradicts the logged plan: the 2026-08-30 entry records the change as terra at `medium`, but sessions run `high` on 09-01 through 09-03 and `medium` on 09-04 through 09-05. (disk) Since Sol, Terra, and Luna are capability tiers rather than speed variants, per that same entry, variant modulates the tier under test rather than acting as an independent second variable. The window measures a combined tier-and-variant change.
- Serena on the orchestrator, terra sessions: `serena_initial_instructions` made 45 calls with zero errors—a per-session handshake, excluded from utilization. (disk) Real calls: 47 with 10 errors (21 percent): `find_symbol` 17 completed / 6 error, `get_symbols_overview` 8 / 2, `search_for_pattern` 7 / 0, `replace_content` 1 completed / 2 error, `get_diagnostics_for_file` 2 / 0, `activate_project` 2 / 0. The write path confirms the 2026-09-01 finding that the permission deny is inert, and additionally shows it mostly failing. Plain read ran approximately 250 calls over the same sessions, so whole-file reads remain the dominant discovery move and the Serena-before-grep append has not displaced them. Caveat: these counts were bounded by model rather than by `time_created`, against the 2026-08-21 rule, so an unknown small share may include the 2026-07-19 session. (session)
- Child sessions are clean delegation, not dreamer contamination: sampled titles across 08-20 through 09-05 are all named seat subagents (`@fixer`, `@explorer`, `@executor`). (disk) Magic Context dream runs do not appear in `opencode.db` as child sessions. Child sessions by day include 09-01 one hundred ten (106 distinct titles), 09-02 thirty, 09-03 eleven, 09-04 thirty-two, 09-05 twenty-two.
- Unverified; flag for window close: 09-04 and 09-05 show roughly 10 child sessions per parent session against the 2026-08-30 baseline of 1.96 children per session for `gpt-5.4`. Small n and partial days; recompute at session grain before treating as a result. (session)
- Magic Context behavioral profile, previously unlogged: `@cortexkit/opencode-magic-context@0.33.1` is pinned in `opencode.json`; its store at `~/.local/share/cortexkit/magic-context/context.db` was created 2026-06-17, predating both the `gpt-5.4` baseline and the terra window, so it is constant across arms. (disk) From the upstream README, it owns context management end to end, requires OpenCode built-in compaction disabled (`compaction.auto` and `compaction.prune` false), disables itself fail-safe if another context manager is active, and checks for three oh-my-opencode hooks it conflicts with (`preemptive-compaction`, `context-window-monitor`, `anthropic-context-window-limit-recovery`). (reported) Whether those hooks exist in oh-my-opencode-slim and whether setup disabled them is not verified. The compaction key is present in `opencode.json`, but its values were not read. (disk)
- The `session.directory` column separates product code from tracer-workflow work, so the standing “re-run on product code before concluding” caveat on the 2026-08-30 cost-per-changed-step table can be answered with a `GROUP BY` rather than by waiting for different work. (disk)

Process note:
- This session re-derived three findings already in STACK.md—the Sep 1 workload confound, Magic Context's presence in the utilization breakdowns, and the staleness of the post-08-18 utilization shares—before reading the file. (session) Second occurrence of the 2026-08-30 process note.
